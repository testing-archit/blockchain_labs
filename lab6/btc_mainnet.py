...
# Step 2: Compute the Public Key (Uncompressed Format)
sk = SigningKey.from_string(private_key, curve=SECP256k1)
vk = sk.get_verifying_key()
x = vk.to_string()[:32].hex()  # X coordinate (32 bytes)
y = vk.to_string()[32:].hex()  # Y coordinate (32 bytes)
uncompressed_public_key = "04" + x + y  # 04 prefix for uncompressed key
print(f"\n Step 2: Uncompressed Public Key:\n {uncompressed_public_key}")

# Step 3: Compute SHA-256 of the Public Key
sha256_hash = hashlib.sha256(bytes.fromhex(uncompressed_public_key)).digest()
print(f"\n Step 3: SHA-256 Hash:\n {sha256_hash.hex()}")
...
# Step 5: Add the Bitcoin Mainnet Version Byte (0x00)
versioned_payload = b'\x00' + public_key_hash
print(f"\n Step 5: Versioned Payload (0x00 + PubKey Hash):\n {versioned_payload.hex()}")
...
# Step 10: Encode Using Base58Check to Get the Final Bitcoin Address
bitcoin_address = base58.b58encode(binary_address).decode()
print(f"\n Bitcoin Address (Base58Check Encoded):\n {bitcoin_address}")